...
class UNet_Pytorch_DeepSup_IFT(torch.nn.Module):
    def __init__(self, n_input_channels=3, n_classes=7, n_filt=64, dim=512, batchnorm=False, dropout=False, upsample="bilinear"):
        super(UNet_Pytorch_DeepSup_IFT, self).__init__()
        self.in_channel = n_input_channels
        self.n_classes = n_classes
        self.use_dropout = dropout
        self.dim = dim
        ...
        # Deep Supervision
        self.output_2 = nn.Conv2d(n_filt * 4 + n_filt * 8, dim, kernel_size=1, stride=1, padding=0, bias=True)
        # 'nearest' a bit faster but results a little worse (~0.4 dice points worse)
        self.output_2_up = nn.Upsample(scale_factor=2, mode=upsample)
        ...
        # Deep Supervision
        self.output_3 = nn.Conv2d(n_filt * 2 + n_filt * 4, dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.output_3_up = nn.Upsample(scale_factor=2, mode=upsample)  # does only upscale width and height
        ...
        # no activation function, because is in LossFunction (...WithLogits)
        self.conv_5 = nn.Conv2d(n_filt, dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.conv_6 = nn.Conv2d(dim, n_classes, kernel_size=1, stride=1, padding=0, bias=True)
    def forward(self, inpt):
        contr_1_1 = self.contr_1_1(inpt)
        contr_1_2 = self.contr_1_2(contr_1_1)
        pool_1 = self.pool_1(contr_1_2)
        ...
        output_4 = output_3_up + conv_5
        final = self.conv_6(output_4)
        return final